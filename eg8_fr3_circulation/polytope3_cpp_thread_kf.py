...
if __name__ == "__main__":
    ...
    # Kalman filter
    kalman_filter_config = test_settings["kalman_filter_config"]
    Q = np.diag(kalman_filter_config["Q"]).astype(config.np_dtype)
    R = np.diag(kalman_filter_config["R"]).astype(config.np_dtype)
    P0 = np.diag(kalman_filter_config["P0"]).astype(config.np_dtype)
    x0 = np.array(info["q"][:7].tolist() + info["dq"][:7].tolist(), dtype=config.np_dtype)
    kalman_filter = KalmanFilter(dt, Q, R, x0, P0)
    ...
    P_EE_0 = np.array([-0.10, 0.26, 1.31])
    P_EE_1 = np.array([-0.07, 0.61, 0.95])
    via_points = np.array([P_EE_0, P_EE_1])
    ...
    measured_joint_angles = np.zeros([horizon, n_controls], dtype=config.np_dtype)
    measured_joint_velocities = np.zeros([horizon, n_controls], dtype=config.np_dtype)
    estimated_joint_angles = np.zeros([horizon, n_controls], dtype=config.np_dtype)
    estimated_joint_velocities = np.zeros([horizon, n_controls], dtype=config.np_dtype)
    ...
    for i in range(horizon):
        ...
        q = info["q"][:7]
        dq = info["dq"][:7]
        q_measured = q + np.random.normal(0, 0.01, 7)
        dq_measured = dq + np.random.normal(0, 0.01, 7)
        if i != 0:
            kalman_filter.predict(ddq)
            kalman_filter.update(np.concatenate([q_measured, dq_measured]))
        ...
        tau = M_mj @ ddq + nle_mj
        finger_pos = 0.01
        info = env.step(tau=tau, finger_pos=finger_pos)
        ...
        measured_joint_angles[i,:] = q_measured
        measured_joint_velocities[i,:] = dq_measured
        estimated_joint_angles[i,:] = q_est
        estimated_joint_velocities[i,:] = dq_est
        controls[i,:] = ddq
        desired_controls[i,:] = ddq_nominal
    ...
    for i in range(7):
        ...
        plt.plot(times, measured_joint_angles[:,i], color="tab:orange", linestyle="-", label="q_{:d}_meas".format(i+1))
        plt.plot(times, estimated_joint_angles[:,i], color="tab:green", linestyle="-", label="q_{:d}_est".format(i+1))
        plt.plot(times, joint_angles[:,i], color="tab:red", linestyle="-", label="q_{:d}".format(i+1))
        plt.legend()
        plt.tight_layout()
    ...
    for i in range(7):
        fig, ax = plt.subplots(figsize=(10,8), dpi=config.dpi, frameon=True)
        plt.plot(times, measured_joint_velocities[:,i], color="tab:orange", linestyle="-", label="dq_{:d}_meas".format(i+1))
        plt.plot(times, estimated_joint_velocities[:,i], color="tab:green", linestyle="-", label="dq_{:d}_est".format(i+1))
        plt.plot(times, joint_velocities[:,i], color="tab:red", linestyle="-", label="dq_{:d}".format(i+1))
        plt.legend()
        plt.tight_layout()
