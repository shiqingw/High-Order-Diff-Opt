...
class SolverNode:
    def __init__(self, ellipsoid_quadratic_coef, A_obs_np, b_obs_np, obs_kappa, vertices, np_dtype, 
                 ...
                 all_dJdq_shm_id, alpha0, gamma1, gamma2, compensation, cbf_id, n_cbf, all_h_shm_id,
    ...
    def solve(self):
        # Get shared memory
        dq = np.memmap(self.dq_shm_id, dtype=self.np_dtype, mode='r', shape=(7,))
        all_J = np.memmap(self.all_J_shm_id, dtype=self.np_dtype, mode='r', shape=(self.n_frames,6,7))
        all_quat = np.memmap(self.all_quat_shm_id, dtype=self.np_dtype, mode='r', shape=(self.n_frames,4))
        all_R = np.memmap(self.all_R_shm_id, dtype=self.np_dtype, mode='r', shape=(self.n_frames,3,3))
        all_P = np.memmap(self.all_P_shm_id, dtype=self.np_dtype, mode='r', shape=(self.n_frames,3))
        all_dJdq = np.memmap(self.all_dJdq_shm_id, dtype=self.np_dtype, mode='r', shape=(self.n_frames,6))
        all_h = np.memmap(self.all_h_shm_id, dtype=self.np_dtype, mode='r+', shape=(self.n_cbf,))
        all_h_dx = np.memmap(self.all_h_dx_shm_id, dtype=self.np_dtype, mode='r+', shape=(self.n_cbf,7))
        all_h_dxdx = np.memmap(self.all_h_dxdx_shm_id, dtype=self.np_dtype, mode='r+', shape=(self.n_cbf,7,7))
        all_phi1 = np.memmap(self.all_phi1_shm_id, dtype=self.np_dtype, mode='r+', shape=(self.n_cbf,))
        all_actuation = np.memmap(self.all_actuation_shm_id, dtype=self.np_dtype, mode='r+', shape=(self.n_cbf,7))
        all_lb = np.memmap(self.all_lb_shm_id, dtype=self.np_dtype, mode='r+', shape=(self.n_cbf,))
        all_ub = np.memmap(self.all_ub_shm_id, dtype=self.np_dtype, mode='r+', shape=(self.n_cbf,))
...
if __name__ == "__main__":
    ...
    dq_shm_id = "/dev/shm/dq_shm"
    all_J_shm_id = "/dev/shm/all_J_shm"
    all_quat_shm_id = "/dev/shm/all_quat_shm"
    all_R_shm_id = "/dev/shm/all_R_shm"
    all_P_shm_id = "/dev/shm/all_P_shm"
    all_dJdq_shm_id = "/dev/shm/all_dJdq_shm"
    all_h_shm_id = "/dev/shm/all_h_shm"
    all_h_dx_shm_id = "/dev/shm/all_h_dx_shm"
    all_h_dxdx_shm_id = "/dev/shm/all_h_dxdx_shm"
    all_phi1_shm_id = "/dev/shm/all_phi1_shm"
    all_actuation_shm_id = "/dev/shm/all_actuation_shm"
    all_lb_shm_id = "/dev/shm/all_lb_shm"
    all_ub_shm_id = "/dev/shm/all_ub_shm"

    # Create the shared memory
    dq_memmap = np.memmap(dq_shm_id, dtype=config.np_dtype, mode='w+', shape=(7,))
    all_J_memmap = np.memmap(all_J_shm_id, dtype=config.np_dtype, mode='w+', shape=(n_selected_BBs,6,7))
    all_quat_memmap = np.memmap(all_quat_shm_id, dtype=config.np_dtype, mode='w+', shape=(n_selected_BBs,4))
    all_R_memmap = np.memmap(all_R_shm_id, dtype=config.np_dtype, mode='w+', shape=(n_selected_BBs,3,3))
    all_P_memmap = np.memmap(all_P_shm_id, dtype=config.np_dtype, mode='w+', shape=(n_selected_BBs,3))
    all_dJdq_memmap = np.memmap(all_dJdq_shm_id, dtype=config.np_dtype, mode='w+', shape=(n_selected_BBs,6))
    all_h_memmap = np.memmap(all_h_shm_id, dtype=config.np_dtype, mode='w+', shape=(n_CBF,))
    all_h_dx_memmap = np.memmap(all_h_dx_shm_id, dtype=config.np_dtype, mode='w+', shape=(n_CBF,7))
    all_h_dxdx_memmap = np.memmap(all_h_dxdx_shm_id, dtype=config.np_dtype, mode='w+', shape=(n_CBF,7,7))
    all_phi1_memmap = np.memmap(all_phi1_shm_id, dtype=config.np_dtype, mode='w+', shape=(n_CBF,))
    all_actuation_memmap = np.memmap(all_actuation_shm_id, dtype=config.np_dtype, mode='w+', shape=(n_CBF,7))
    all_lb_memmap = np.memmap(all_lb_shm_id, dtype=config.np_dtype, mode='w+', shape=(n_CBF,))
    all_ub_memmap = np.memmap(all_ub_shm_id, dtype=config.np_dtype, mode='w+', shape=(n_CBF,))
    ...
    for (i, bb_key) in enumerate(selected_BBs):
        for (j, obs_key) in enumerate(obs_col.face_equations.keys()):
            ...
            solver = SolverNode(ellipsoid_quadratic_coef, A_obs_np, b_obs_np, obs_kappa, vertices, np_dtype,
                                ...
                                alpha0, gamma1, gamma2, compensation, cbf_id, n_CBF, all_h_shm_id,
                                all_h_dx_shm_id, all_h_dxdx_shm_id, all_phi1_shm_id, all_actuation_shm_id, 
                                all_lb_shm_id, all_ub_shm_id)
        ...
        try:
            for i in range(horizon):
                ...
                if CBF_config["active"]:
                    ...
                    all_h = np.memmap(all_h_shm_id, dtype=config.np_dtype, mode='r', shape=(n_CBF,))
                    # all_h_dx = np.memmap(all_h_dx_shm_id, dtype=config.np_dtype, mode='r', shape=(n_CBF,7))
                    # all_h_dxdx = np.memmap(all_h_dxdx_shm_id, dtype=config.np_dtype, mode='r', shape=(n_CBF,7,7))
                    all_phi1 = np.memmap(all_phi1_shm_id, dtype=config.np_dtype, mode='r', shape=(n_CBF,))
                    all_actuation = np.memmap(all_actuation_shm_id, dtype=config.np_dtype, mode='r', shape=(n_CBF,7))
                    all_lb = np.memmap(all_lb_shm_id, dtype=config.np_dtype, mode='r', shape=(n_CBF,))
                    all_ub = np.memmap(all_ub_shm_id, dtype=config.np_dtype, mode='r', shape=(n_CBF,))
