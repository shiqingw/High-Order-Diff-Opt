...
perturb_A_original = 0.1*torch.rand(3*4//2, dtype=config.torch_dtype, requires_grad=True)
matrix = torch.zeros((3, 3), dtype=config.torch_dtype)
indices = torch.triu_indices(3, 3)
matrix[indices[0], indices[1]] = perturb_A_original
matrix = matrix
perturb_A = torch.matmul(matrix, matrix.transpose(-1, -2)) + 0.01*torch.eye(3, dtype=config.torch_dtype)
...
indices = torch.triu_indices(3, 3)
perturb_A_sym = perturb_A[indices[0], indices[1]]
perturb = torch.cat([perturb_A_sym.view(-1), perturb_a.view(-1)]).view(1,9,1)
