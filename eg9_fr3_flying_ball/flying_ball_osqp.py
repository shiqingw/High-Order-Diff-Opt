...
if __name__ == "__main__":
    ...
    for i in range(horizon):
        ...
        if CBF_config["active"]:
            ...
            all_P_obs_np[0, :] = info["ball_pos"].astype(config.np_dtype)
            all_quat_obs_np[0, :] = np.array([0, 0, 0, 1], dtype=config.np_dtype)
            all_v_obs_np[0, :] = info["ball_vel"].astype(config.np_dtype)
            all_omega_obs_np[0, :] = np.zeros(3, dtype=config.np_dtype)
            all_v_dot_obs_np[0, :] = np.array([0, 0, -9.81], dtype=config.np_dtype)
            all_omega_dot_obs_np[0, :] = np.zeros(3, dtype=config.np_dtype)
