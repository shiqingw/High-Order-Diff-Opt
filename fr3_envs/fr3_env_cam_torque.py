...
from cores.utils.render_utils import cvPose2BulletView, cvK2BulletP
...
class FR3CameraSim(Env):
    ...
    def __init__(self, camera_config, enable_gui_camera_data, base_pos=[0,0,0], base_quat=[0,0,0,1], render_mode= None, record_path=None):
        ...
        # Enable the torque control on the joints by turning of velocity control (max=0)
        p.setJointMotorControlArray(
            bodyIndex=self.robotID,
            jointIndices=self.active_joint_ids,
    ...
    def reset(
        ...
        q, dq, applied_torque = self.get_state()
        self.update_pinocchio(q, dq)
        info = self.get_info(q, dq, applied_torque)
    ...
    def get_info(self, q, dq, applied_torque):
        """
        info contains:
        -------------------------------------
        q: joint position
    ...
    def get_dynamics(self, q, dq):
        ...
        # drift = np.vstack((dq[:, np.newaxis], -Minv @ nle[:, np.newaxis])) # f.shape = (18, 1)
        # actuation = np.vstack((np.zeros((9, 9)), Minv)) # g.shape = (18, 9)

        return M, Minv, nle

    def step(self, action, return_image=False):

        self.send_joint_command_torque(action)
        p.stepSimulation()
    ...
    def get_state(self):
        q = np.zeros(9)
        dq = np.zeros(9)
        applied_torque = np.zeros(9)

        for i, id in enumerate(self.active_joint_ids):
            _joint_state = p.getJointState(self.robotID, id)
            q[i], dq[i], applied_torque[i] = _joint_state[0], _joint_state[1], _joint_state[3]

        return q, dq, applied_torque
    ...
    def send_joint_command_torque(self, torque):
        p.setJointMotorControlArray(
            bodyIndex=self.robotID,
            jointIndices=self.active_joint_ids,
