...
from cores.utils.rotation_utils import get_quat_from_rot_matrix, get_Q_matrix_from_quat, get_dQ_matrix, \
    get_homogenous_matrix, inverse_homogenous_matrix
from cores.utils.control_utils import get_torque_to_track_traj_const_ori
from cores.configuration.configuration import Configuration
from cores.utils.camera_utils import get_camera_intrinsic_matrix, project_points_from_world_to_pixel, \
    project_points_from_pixels_to_world
...
if __name__ == "__main__":
    ...
    intrinsic_matrix = get_camera_intrinsic_matrix(env.model, camera_name, image_height, image_width)
    ...
    for i in range(horizon):
        ...
        rgb_image = env.get_rgb_image(camera_name)
        ...
        P_CAM = info["P_CAMERA"]
        R_CAM = info["R_CAMERA"]
        H_cam_to_world = get_homogenous_matrix(R_CAM, P_CAM)
        H_world_to_cam = inverse_homogenous_matrix(H_cam_to_world)
        point = np.array([0.1,0.2,0.88])
        pixel = project_points_from_world_to_pixel(point, intrinsic_matrix, H_world_to_cam, image_width, image_height)
        pixel = np.squeeze(pixel)
        cv2.circle(bgr_image, (pixel[0], pixel[1]), 10, (0, 255, 0), -1)
        # cv2.imwrite(results_dir+'/output_image_{:04d}.png'.format(i), bgr_image)
