...
p_vars = [p1, p2]
theta_vars = [theta, a1, a2]
dummy_vars = [d11, d22]
p_vec_sp = sp.Matrix([p1, p2])
a_vec_sp = sp.Matrix([a1, a2])
D_sp = sp.Matrix([[d11, 0],
                [0, d22]])
...
p_val = np.random.rand(2)
theta_val = np.random.rand()
a_val = np.random.rand(2)
d11_val = np.random.rand()
d22_val = np.random.rand()
D_val = np.array([[d11_val, 0],
                [0, d22_val]])
R_val = np.array([[np.cos(theta_val), -np.sin(theta_val)],
...
print("##############################################")
alpha, alpha_dx, alpha_dxdx = DOC.getGradientAndHessianEllipseAndLogSumExp(p_val, a_val, theta_val, D_val, R_val, B_np, b_np, kappa)
print('alpha: ', alpha)
print('alpha_dx: ', alpha_dx)
print('alpha_dxdx: ', alpha_dxdx)

print("##############################################")
x_val = np.concatenate((np.array([theta_val]), a_val))
dummy_val = np.array([d11_val, d22_val])
