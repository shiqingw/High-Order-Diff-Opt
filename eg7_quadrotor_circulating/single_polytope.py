...
if __name__ == '__main__':
    ...
    dt = system.delta_t
    env = SkydioMuJocoEnv(xml_name="scene_single_polytope", cam_distance=cam_distance, cam_azimuth=cam_azimuth,
        ...
        if CBF_config["active"]:
            ...
            for (kk, key) in enumerate(obs_col.face_equations.keys()):
                ...
                if min_distance < distance_threshold:
                    ...
                    C[1,:] = tmp
                    lb[1] = C[1,:] @ system.ueq + 0.5 - 10*CBF
                    ub[1] = np.inf
                else:
                    CBF = 0
                    phi1 = 0
            ...
            g = -H @ u_nominal
            C[n_CBF+1:n_CBF+n_controls+1,:] = np.eye(n_controls, dtype=config.np_dtype)
            lb[n_CBF+1:] = input_lb
            ub[n_CBF+1:] = input_ub
